...
function oauthCallback(req) {
  ...
  if (req.response.status == 401) {
    ...
    var error_msg_str = parser('error_description', req.response.headers['WWW-Authenticate']);

    if (error_str !== undefined && error_str === "invalid_token" ){ 
            if ( error_msg_str !== undefined && error_msg_str === "expired_token" && last_exten != null) {
                // if token has expired and this is he first try, we ask a new one
